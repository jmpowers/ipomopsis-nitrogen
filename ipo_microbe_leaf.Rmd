...
exp2leaf.data$Name <- recode(exp2leaf.data$Name, !!!shortnames)
...
These are the counts of the samples.
...
vol <- vol / as.numeric(meta$equil + meta$pumping)/3600 / meta$leaves #0.5 hr of equilibration plus 0.25 hr pumping, one flower
```
...
```{r ordination}
(cap_treat <- vegan::capscale(sqrt(vol) ~ trt_col + rundate, data=meta))
...
cap_dates <- vegan::capscale(sqrt(vol) ~ rundate, data=meta)
...
plot(x=factor(meta$rundate), y=vol$`2-Pentanol, 3-chloro-4-methyl-, (R*,S*)-(.+/-.)-`)
```
