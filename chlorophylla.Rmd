...
field.treatment <- read_csv("data/NEXP1_field_treatments - 2022.csv") %>% mutate(treatment = as.factor(treatment), site=as.factor(site))
...
greenhouse.wavelength$species <- factor(greenhouse.wavelength$species, levels = c("agg", "ten")) #agg is on left facet panel, ten is right side

Chla <- ggplot(greenhouse.wavelength, aes(x= species, y= chlorophylla, color=treatment)) + 
...
print(Chla)
ggsave(Chla, file='plots-nitrogen/greenhouse_Chlorophylla.jpg', width=150, height=140, units= 'mm', dpi=600)
...
field.wavelength$species <- factor(field.wavelength$species, levels = c("Lagg", "Ihyb")) #Lagg is on left facet panel, ihyb is right side
field.wavelength$treatment <- factor(field.wavelength$treatment, levels = c("nitrogen", "watercontrol", "control")) #orders nitrogen levels in the figure legend
