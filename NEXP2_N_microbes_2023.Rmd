...
#manually edited filenames, this reads in the new google sheet
...
# qual cap
```{r}
(cap_treat <- vegan::capscale(sqrt(vol.qual) ~ nitrogen * time * microbe, data=meta))
...
mega.glht <- bind_cols(meta,vol.qual) %>%  #combines meta and vol.qual
  ...
  mutate(summary.glht= map(glht.every.vol, ~multcomp::summary(.x, test=adjusted(type="fdr"))))   #summary.glht has all pvalues
...
multcomp::summary(glht.T.N.M, test=adjusted(type="none"))
...
AverageVOCcapM.N.T <- capscale(sqrt(averagetable) ~ microbe * nitrogen * time, data = averagemetadata, distance = "bray")
...
ggplot(averagePlant, aes(color=paste(microbe, nitrogen), x=time, y=indole))+geom_boxplot()+scale_y_sqrt() #box plot for average plant indole 
...
library(ggvegan)
library(ggrepel)
cap.night.dataframe <- fortify(average.night.cap)
  ...
  #labs(subtitle="ANOVA; p=0.264",hjust = 1)+ #adds subtitle at top of figure
   theme(legend.title = element_text(size=12, color = "black"), legend.text=element_text(size=10),
    ...
    geom_text(data=cap.night.dataframe %>% filter(Score=="biplot", str_detect(Label, ":")) %>% #labels text to arrows
            mutate(Label=c("Low Normal", "Low Sterile")),
...
ggsave(average.night.ordination, file="plots-microbe/night.cap.svg", width= 7, height=8)
...
summary(glht.T.N.M, test=adjusted(type="none"))
