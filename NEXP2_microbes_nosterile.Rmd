...
axis.eig.day <- eig/ sum(eig)
print(axis.eig)

anova(average.day.cap, by="term")

cap.day.dataframe <- fortify(average.day.cap) #makes a dataframe for ggplot to use
average.day.ordination = ggplot(bind_cols(filter(cap.day.dataframe, score=="sites"),averagedaymetadata), 
 ...
 # ggtitle("I. aggregata Nighttime Volatiles") +
  ...
  ylab(paste0("CAP2 (", round(100*axis.eig.day[2]), "% explained)")) + 
  #custom CAP axes: rounds and multiplies the eigenvalue above for % explained
  xlab(paste0("CAP1 (", round(100*axis.eig.day[1]), "% explained)")) + 
 # scale_color_manual(name="Nitrogen Level", labels= c("High", "Low"), values=c("tan2","cadetblue3")) +
  #labs(subtitle="ANOVA; p=0.264",hjust = 1)+ #adds subtitle at top of figure
   theme(legend.title = element_text(size=12, color = "black"), legend.text=element_text(size=11),
           legend.justification=c(0,1), 
           legend.position="right",  #c(0.90, 0.32)
...
print(average.day.ordination)
ggsave(average.day.ordination, file="plots-microbe-nosterile/day.cap.png", width= 6, height=6)
...
average.night.ordination = ggplot(bind_cols(filter(cap.night.dataframe, score=="sites"),averagenightmetadata), 
 ...
 # scale_color_manual(name="Nitrogen Level", labels= c("High", "Low"), values=c("tan2","cadetblue3")) +
  #labs(subtitle="ANOVA; p=0.264",hjust = 1)+ #adds subtitle at top of figure
   theme(legend.title = element_text(size=12, color = "black"), legend.text=element_text(size=11),
           legend.justification=c(0,1), 
           legend.position="right", #c(0.77, 0.62)
           legend.background = element_blank(),
           legend.key = element_blank(),
            plot.subtitle = element_text(hjust = 1))+ #moves subtitle to right side 
    geom_segment(data=cap.night.dataframe %>% filter(score=="biplot", str_detect(label, ":")), #adds arrows 
...
ggsave(average.night.ordination, file="plots-microbe-nosterile/night.cap.png", width= 6, height=6)
...
peak.area.plot <- bind_cols(meta,vol.qual) %>%  
...
ggsave(peak.area.plot, file="plots-microbe-nosterile/peak.area.plot.png", width= 15, height=25)
...
eig.rel <-eigenvals(rel.amount.cap)
axis.eig.rel <- eig.rel/ sum(eig.rel)
print(axis.eig.rel)
...
rel.amount.ordination = ggplot(bind_cols(filter(rel.amount.dataframe, score=="sites"),meta), aes(x=CAP1, y= CAP2, color = paste(nitrogen, microbe), shape=time)) + 
 ...
 # ggtitle("I. aggregata Daytime Volatiles") +
  theme(plot.title = element_text(hjust = 0.5))+
 ylab(paste0("CAP2 (", round(100*axis.eig.rel[2]), "% explained)")) + 
  #custom CAP axes: rounds and multiplies the eigenvalue above for % explained
  xlab(paste0("CAP1 (", round(100*axis.eig.rel[1]), "% explained)")) + 
 # scale_color_manual(name="Nitrogen Level", labels= c("High", "Low"), values=c("tan2","cadetblue3")) +
  #labs(subtitle="ANOVA; p=0.264",hjust = 1)+ #adds subtitle at top of figure
   theme(legend.title = element_text(size=12, color = "black"), legend.text=element_text(size=10),
           legend.justification=c(0,1), 
           legend.position="right", #c(0.03, 0.95)
           legend.background = element_blank(),
           legend.key = element_blank(),
            plot.subtitle = element_text(hjust = 1)) #moves subtitle to right side 
...
ggsave(rel.amount.ordination, file="plots-microbe-nosterile/rel.amount.cap.png", width= 8, height=6)
