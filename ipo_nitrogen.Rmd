...
exp1_field_meta <- read.csv("data/2022 Poverty Gulch Volatiles.csv") %>%
  mutate(plant = as.character(plant),
         Sample=paste0(plant, ifelse(is.na(replicate),"", replicate)), 
         type= factor(ifelse(str_detect(Sample, "AIR"), "ambient", "floral"))) 
  #^ 
...
exp1_field_verdicts <- gc_verdicts %>% filter(str_detect(batch,"PG_")) %>% separate(sample2, into=c("site", "plant", "d_n", "date", "run_date", "gc_n", "file"), remove=FALSE) %>% 
  filter(site%in% c("Lagg", "Ihyb")) %>%  #sorts verdicts table by only Lagg or Ihyb samples (gets rid of the rest)
   mutate(date=ymd(date)) #changes date to match formatting on both

exp1_field_verd_merged <- left_join(exp1_field_verdicts, exp1_field_meta) 
