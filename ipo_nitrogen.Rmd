...
NEXP1.theme <- theme(
      ...
      #facets
                  ...
                  strip.text = element_text(size = 16, face="italic") )       #This changes the size of the facet labels
...
# Convert ordination scores to data frame
ordination_scores <- as.data.frame(cap_treat$CCA$v)  
#need to add speices and time to cap_treat

#Ordination ggplot
greenhouse_ordination <- ggplot(cap_treat, aes(x = CAP1, y = CAP2, color = species, shape = time)) +
  geom_point(size = 2.6) +
  theme_classic() +
  coord_fixed(xlim = c(-1.5, 2.9), ylim = c(-1, 2.4)) +
  geom_hline(yintercept = 0, linetype = "dotted") +
...
greenhouse.ordination = ggplot(bind_cols(filter(cap_treat, score=="sites"),meta), aes(x=CAP1, y= MDS1, color = treatment)) 
  geom_point(size=2.6) +
  theme_classic() +
  coord_fixed(xlim=c(-1.5, 2.9), ylim=c(-1, 2.4)) +
  geom_hline(yintercept = 0, linetype="dotted") +
...
plot.total.emissions.field=
  ...
  geom_point (position=position_dodge(width=0.9))+ #makes it so point don't overlap
  scale_y_sqrt()+
  ylab("Greenhouse Total Emissions")+
  xlab("") +
  facet_wrap(vars(species)) +
  facet_grid(. ~ species, labeller=labeller(species = facet.labs)) + 
  scale_x_discrete(labels=c('Day', 'Night', 'Day','Night')) + #x axis text
