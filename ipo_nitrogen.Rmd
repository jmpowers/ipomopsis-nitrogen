...
quant.full <- read.shimadzu.quant("data/NEXP1_field_greenhouse_quant.txt") %>% #reads in shimadzu text file with quant integrations
...
```{r}
#TODO: Fix code lol 
#betadisper(vegdist(vol), meta$plantid)
#count(plantid)
```
...
anova(Field.AverageVOCcapS.T.T)

#cap Treatment*Time p=
Field.AverageVOCcapT.T <- capscale(sqrt(averagetable.field) ~ treatment * time, data = averagemetadata.field, distance = "bray")

anova(Field.AverageVOCcapT.T)

#cap Treatment p=
Field.AverageVOCcapTreatment <- capscale(sqrt(averagetable.field) ~ treatment, data = averagemetadata.field, distance = "bray")

anova(Field.AverageVOCcapTreatment)

#cap Time p=0.001***
Field.AverageVOCcapTime <- capscale(sqrt(averagetable.field) ~ time, data = averagemetadata.field, distance = "bray")

anova(Field.AverageVOCcapTime)
```
...
plot.indole.agghyb.field=
  ggplot(averageHYB.field, aes(color=treatment, x=time,y=indole))+
  #geom_boxplot(position=position_dodge(width=0.9))+
  #geom_point (position=position_dodge(width=0.9))+
  geom_boxplot()+
  scale_y_sqrt()+
  ylab("Indole Emissions Hybrids")+
  #ylim(0,25000) + #scales y-axis 
  facet_wrap(vars(species))+ #makes two columns for species
    facet_grid(. ~ species, labeller=labeller(species = facet.labs))+ 
    theme(panel.border = element_blank(),
    ...
    theme(legend.title = element_text(size=11, color = "black"),
        legend.text=element_text(size=9))
print(plot.indole.agghyb.field)    
ggsave(plot.indole.agghyb.field, file='Indole_boxplot.jpg', width=150, height=110, units= 'mm', dpi=300)
...
#TODO: change for field exp
#Only indole lmer with plantid as random effect

# noaverage$S.T.T.field <- factor(paste0(noaverage.field$species, noaverage$time, noaverage.field$treatment))
# (indolelmer.field<-lmer(sqrt(indole)~S.T.T.field + (1|plantid), data=noaverage.field)) #use not averaged data! d
# summary(multcomp::glht(indolelmer.field, linfct= multcomp::mcp(S.T.T.field =TukeyHSD(idkwhat to put here)))

```

### START HERE (fix indole)
