...
agg.Day.ordination = ggplot(bind_cols(filter(cap.dataframe, score=="sites"),averageaggmetadata), aes(x=CAP1, y= MDS1, color = treatment)) + 
 ...
 # ggtitle("I. aggregata Daytime Volatiles") +
  ...
  labs(subtitle="ANOVA; p=0.037",hjust = 1)+ #adds subtitle at top of figure
...
#emmeans table 
total.emmeans<-emmeans(total.emission.model,c("time")) 
...
plot.total.emissions.pretty=
  ggplot(meta, aes( y= totalemission, x=str_to_title(species),
  color=str_to_title(treatment) %>% factor(levels=c("Low", "High"))))+ #%>% 
   # fct_recode(`Water control`="Watercontrol")))+ 
  geom_boxplot(position=position_dodge(width=0.9))+
  scale_y_sqrt()+
  ...
  labs(color="Treatment")+
   scale_color_manual(values = c(High="orange",Low="skyblue"))+
  facet_wrap(vars(time), labeller = as_labeller(c(D="Day", N="Night")))

print(plot.total.emissions.pretty)
ggsave(plot.total.emissions.pretty, file='greenhouse_totalemissions.jpg', width=150, height=140, units= 'mm', dpi=300)
```
...
#emmeans table 
field.total.emmeans<-emmeans(total.emission.model.field, c("time")) 
...
plot.total.emissions.field.pretty=
  ggplot(meta.field, aes( y= totalemission, x=str_to_title(species),
  color=str_to_title(treatment) %>% factor(levels=c("Control", "Watercontrol", "Nitrogen")) %>% 
    fct_recode(`Water control`="Watercontrol")))+ 
  geom_boxplot(position=position_dodge(width=0.9))+
  scale_y_sqrt()+
  ylab("Total Emissions")+
  xlab("Species") +
    ...
    theme(legend.title = element_text(size=11, color = "black"), panel.grid.major.x=element_blank(),
        legend.text=element_text(size=9), legend.position = "bottom")+
  labs(color="Treatment")+
   scale_color_manual(values = c(Nitrogen="orange",Control="darkgreen",`Water control`="skyblue"))+
  facet_wrap(vars(time), labeller = as_labeller(c(D="Day", N="Night")))

print(plot.total.emissions.field.pretty)
ggsave(plot.total.emissions.field.pretty, file='field_totalemissions.jpg', width=150, height=140, units= 'mm', dpi=300)
```
...
ggplot(dominantvocs, aes(y=fct_reorder(name, value), x=value, color=treatment, shape=time))+
    ...
    scale_color_manual(values=c("orange","skyblue"))+ #manually change colros
...
ggplot(dominantvocs.field, aes(y=fct_reorder(name, value), x=value, color=treatment, shape=time))+
    ...
    scale_x_sqrt()+ #scales x axis so you can see small peak area dots
    facet_wrap(vars(species))+ #makes two pannels, agg and ten
    scale_color_manual(values=c("pink","orange","skyblue"))+ #manually change colros
    theme_minimal() #removed grey from background
...
rel.amount.ordination.field = ggplot(bind_cols(filter(rel.amount.dataframe.field, score=="sites"),meta.field), aes(x=CAP1, y= MDS1, color = treatment)) + 
 ...
 # ggtitle("I. aggregata Daytime Volatiles") +
  ...
  ylab("MDS1 (% explained)") + 
  xlab("CAP1 (% explained)") + 
  scale_color_manual(name="Nitrogen Level", labels= c("Control", "Nitrogen", "watercontrol"), values=c("pink","orange","skyblue")) + 
