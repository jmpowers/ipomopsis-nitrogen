...
metadata <- metadata %>% mutate(type= ifelse(species=="AMB", "ambient","floral")) %>% rename(trap=sample) %>% as.data.frame() %>% 
  load_metadata(date = "date", sample = "filename", group = "species", type = "type") #combine species and time (previously separate)
...
unknownterpenes <- c("tricyclo[5.2.1.02,6]dec-2-ene"= "monoterpene A","3-ethenyl-1,2-dimethylcyclohexa-1,4-diene"= "monoterpene B","cosmene"= "monoterpene C", "p-mentha-1,3,8-triene"="monoterpene D")
...
quant.full %>% select(Dirname, Filename, Name, Area) %>%
  ...
  mutate(Name = dplyr::recode(Name, !!!unknownterpenes )) %>%  #renames retention time puzzel compounds to monoterpene A-D
...
(need_quant_both <- setdiff(setdiff(voclist, names(unknownterpenes)), quantlist))
...
vol <- vol[rownames(vol.qual), c(intersect(voclist,colnames(vol)), unknownterpenes)] # rownames puts in same order as vol.qual; voclist filters list down to compounds only found in either greenhouse & field. Adds monoterpenes we removed back in
