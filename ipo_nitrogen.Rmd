...
unknownterpenes <- c("tricyclo[5.2.1.02,6]dec-2-ene"= "monoterpene A",
                     "3-ethenyl-1,2-dimethylcyclohexa-1,4-diene"= "monoterpene B",
                     "cosmene"= "monoterpene C",
                     "p-mentha-1,3,8-triene"="monoterpene D")

# load short names
ipochems <- read_csv("data/Ipo volatile compounds - chemsf_ipo.csv") %>% 
  select(name, shortname, standard, verdict) %>%  filter(verdict != "") %>% 
  mutate(shortname = recode(shortname, !!!unknownterpenes))
...
field.label.colors <- scale_color_manual(name="Nitrogen Level", 
          labels= c(control="No water added", nitrogen= "High nitrogen", watercontrol= "Low nitrogen"), 
           values=c(control= "#457A26",nitrogen="#E69F00", watercontrol= "#0072B2"))

greenhouse.label.colors <- scale_color_manual(name="Nitrogen Level", 
          labels= c( high= "High nitrogen", low= "Low nitrogen"), 
          values=c(high="#E69F00", low="#0072B2"))

# opt 1 = "#F0E442", "#0072B2" ,"#D55E00"
# opt 2 = "#E69F00", "#CC79A7", "#009E73"
# opt 4 = "#0072B2", "#E69F00", "#009E73"
# opt 5 = "#DDA0DD", "#FFA07A", "#008080"
 #opt 6 = "#E69F00", "#56B4E9", "#999999"
# opt 7 = "#6C8037", "#8E3972", "#2B75A5"
 #opt 8 = "#A3C653", "#BF4080" ,"#2B75A5"
# opt 10 ="#F9A620","#457A26" ,"#8CCBE2" 
# 
# "#9B59B6",nitrogen="#F9A620", watercontrol= "#0072B2"

#order figure legend (must put before each plot is made)
#df$treatment <- factor(df$treamtent, levels = c("High nitrogen", "Low nitrogen", "No water added"))
...
#TODO: NA for filter_ambient_ratio (look at figure)
chemtable$filter_final <- with(chemtable, filter_RT == "OK" & RT.var < 2 & filter_match =="OK" & 
                                 ((filter_freq.agg == "OK" & ambient_ratio_agg>4) | 
                                    (filter_freq.ten == "OK" & ambient_ratio_ten>4)))
...
meta <- metadata[metadata$type == "floral" & !(metadata$sample %in% files_exclude),] %>% droplevels() %>% 
            mutate(species=factor(species, levels=c("agg", "ten"))) %>% #orders level for species, agg always comes first
            mutate(treatment=factor(treatment, levels=c("high", "low")))  #orders treatment-- high first then low N
...
chemtable.field$filter_final <- with(chemtable.field, filter_RT == "OK" & RT.var < 2 & filter_match =="OK" & 
                                 (  (filter_freq.Lagg == "OK" & ambient_ratio_Lagg>4) | 
                                    (filter_freq.Ihyb == "OK" & ambient_ratio_Ihyb>4) ))
...
meta.field <- metadata.field[metadata.field$type == "floral" & !(metadata.field$sample %in% files_exclude),-48] %>%#48 is the second sample colum (skips)
  ...
  mutate(species=factor(species, levels=c("Lagg", "Ihyb"))) %>% #orders level for species, agg always comes first
            mutate(treatment=factor(treatment, levels=c("nitrogen", "watercontrol", "control")))  #orders treatment-- high first then low N
...
```{r quant}
##NOTE: can only run this section once bc it replaces vol and vol.field (must re-run section above before re-running this section)
...
quantlist <- colnames(vol.exp1.exp1field)[-1]
(need_quant_both <- setdiff(voclist, quantlist)) 
# Passed bouquet but don't have quant for these compounds
# JP determined that these six are misidentified column contaminants, or nylon bag residue - don't include
...
vol <- vol[rownames(vol.qual), intersect(voclist,quantlist)] # rownames puts in same order as vol.qual; voclist filters list down to compounds only found in either greenhouse & field. Adds monoterpenes we removed back in
...
vol.field <- vol.field[rownames(vol.qual.field), intersect(voclist,colnames(vol))] 
...
standard_shortnames <- c(Indole="indole", Linalool="linalool", alpha_Pinene="a-pinene", Hexenol="(E)-hex-3-en-1-ol", Caryophyllene="b-caryophyllene", Methyl_salicylate="methyl 2-hydroxybenzoate")

#Converting to ng/hr uses standards for 2022 on the GCMS
ipochemsf <-  ipochems %>% mutate(standard = standard_shortnames[standard]) %>% #ipo chemicals filtered
  left_join(
    #reads in spreadsheet with standard slopes in from johns github
    read_csv(url("https://raw.githubusercontent.com/jmpowers/RMBL-GCMS/main/output/standards/regression_slopes.csv")) %>%
      filter(year == "2022b_newflowpath")) %>% 

  distinct(shortname, .keep_all = TRUE)


vol <- sweep(vol, 2, left_join(tibble(shortname=colnames(vol)),ipochemsf)$area_per_ng, FUN = '/')
vol.field <- sweep(vol.field, 2, left_join(tibble(shortname=colnames(vol.field)),ipochemsf)$area_per_ng, FUN = '/')
...
megameta <- bind_rows(greenhouse=meta, .id = "experiment", field=meta.field %>% mutate(treatment=as.character(dplyr::recode(treatment,`watercontrol` = "low",control="low", nitrogen="high")), time=as.character(time), species= as.character(dplyr::recode(species, Ihyb="ten", Lagg="agg")))) #Ihyp = ten bc we were trying to simplify the figure (too many species), grouped ten and hyb vs aggs
...
# best <- list()
# i <- 1
# vol.left <- bind_rows(sampletable, sampletable.field)[,need_quant_both]^(1/4) # halfway between areas and presence-absence
...
#   i <- i+1
# }
# (best.vol <- as.data.frame(t(bind_rows(sampletable, sampletable.field)[unlist(best),need_quant_both])) %>% rownames_to_column("name"))
# chemtable %>% filter(name %in% need_quant_both) %>% select(name, RT, RT.var) %>% arrange(RT)
```
...
longdata.RT <- bind_rows(longdata, longdata.field) %>% filter(name %in% voclist)
ggplot(longdata.RT, aes(x=RT, y=fct_reorder(name, RT))) + 
  geom_jitter(aes(color=factor(name, levels=sample(levels(name)))), width = 0, height=0.5, size=0.5) + 
  geom_point(data=quant.RT %>% filter(name %in% voclist)) + 
  guides(color="none") + ylab("") + theme_minimal()

load("data/get_kovats.rda")
longdata.RT %>% filter(name %in% need_quant_both) %>% summarize(RT = median(RT), .by=name) %>% 
  mutate(kovats=round(get_kovats(RT, "2023-08-15")))
...
longdata.RT <- bind_rows(longdata, longdata.field) %>% filter(name %in% c(colnames(vol.qual), colnames(vol.qual.field))) %>%
...
anova.cca(cap_treat_field, by="term") # samples should be averaged by plant first
...
#adds average total emissions column to averagePlant -- for total emissions boxplot
averagePlant.total <- averagePlant %>% ungroup() %>% mutate(total= rowSums(averagePlant[,12:33]))
...
#adds average total emissions column to averagePlant -- for total emissions boxplot
averagePlant.total.field <- averagePlant.field %>% ungroup() %>% mutate(total= rowSums(averagePlant.field[,14:35]))
...
averagePlant$species <- factor(averagePlant$species, levels = c("agg", "ten")) #agg is on left facet panel, ten is right side
...
plot.indole.aggten=
  ...
  xlab("") +
  #facet_wrap(vars(species)) +
  #facet_wrap(. ~species, labeller = labeller(species = c("agg" = expression(italic("I. aggregata")), "ten" = expression(italic("I. tenuituba"))))) +
  facet_grid(. ~ species, labeller=labeller(species = facet.labs))+ 
  scale_x_discrete(labels=c('Day', 'Night', 'Day','Night'))+ #x axis text
  #ylim(0.1, 0.56) +  # Set y-axis limits from 0.1 to 0.56  
   NEXP1.theme +
...
ggsave(plot.indole.aggten, file='plots-nitrogen/Indole_boxplot.jpg', width=150, height=110, units= 'mm', dpi=600)
...
ggsave(plot.indole.Lagghyb.field, file='plots-nitrogen/Indole_field_agghyb_boxplot.jpg', width=150, height=110, units= 'mm', dpi=600)
...
Day.ordination = ggplot(bind_cols(filter(cap.day.dataframe, score=="sites"),averagemetadata.DAY), aes(x=CAP1, y= CAP2)) + 
  geom_point(size=2.6, aes(color = treatment, shape = species)) +
   
  #Compound names on CAP:
  geom_text_repel(data=cap.day.dataframe %>% 
                    left_join(ipochems, by=c("label"="name")) %>% #adds compound name 
              filter(score=="species", CAP1^2 +CAP2^2>0.1^2) %>%  #gets rid of compounds clusterd in middle of cap; EDIT THIS ON FINAL FIGURE**
              mutate(label=greekify(label)),
              aes(x=CAP1*6, y=CAP2*6, label=label), size=3) +
  
  
  # Add ellipses for group dispersion
  stat_ellipse(aes(color = treatment, fill = treatment), alpha = 0.2, geom = "polygon") +
  theme_classic() + 
  geom_hline(yintercept = 0, linetype="dotted") + 
  geom_vline(xintercept = 0, linetype="dotted") +
  labs(color = "Treatment", fill = "Treatment") +
  theme(plot.title = element_text(hjust = 0.5))+
  ylab("CAP2 (4.4% explained)") + 
...
ggsave(Day.ordination, file='plots-nitrogen/Day_ordinationplot.jpg', width=130, height=105, units= 'mm', dpi=600)
...
##generate a plot in ggplot
Night.ordination = ggplot(bind_cols(filter(cap.night.dataframe, score=="sites"),averagemetadata.NIGHT), aes(x=CAP1, y= CAP2)) + 
  geom_point(size=2.6, aes(color = treatment, shape = species))+
   #Compound names on CAP:
  geom_text_repel(data=cap.day.dataframe %>% 
                    left_join(ipochems, by=c("label"="name")) %>% #adds compound name 
 ...
 # ggtitle("I. aggregata Night Volatiles") +
  theme(plot.title = element_text(hjust = 0.5))+
  ylab("CAP2 (2.4% explained)") + 
  xlab("CAP1 (3.1% explained)") + 
  #labs(shape = "Species") +
  scale_color_manual(name="Nitrogen Level", 
          labels= c( high= "High nitrogen", low= "Low nitrogen"), 
          values=c(high="#E69F00", low="#0072B2"),
...
ggsave(Night.ordination, file='plots-nitrogen/Night_ordinationplot.jpg', width=130, height=105, units= 'mm', dpi=600)
...
##generate a plot in ggplot
Day.ordination.field = ggplot(bind_cols(filter(cap.day.dataframe.field, score=="sites"),averagemetadata.DAY.field), aes(x=CAP1, y= CAP2)) + 
  geom_point(size=2.6, aes(color = treatment, shape = species)) + #move color and shape here bc geom_text no treatment
  #Compound names on CAP:
  geom_text_repel(data=cap.day.dataframe %>% 
                    left_join(ipochems, by=c("label"="name")) %>% #adds compound name 
              filter(score=="species", CAP1^2 +CAP2^2>0.1^2) %>%  #gets rid of compounds clusterd in middle of cap; EDIT THIS ON FINAL FIGURE**
 ...
 # ggtitle("I. aggregata Daytime Volatiles") +
  ...
  scale_shape_manual(
    ...
    labels = c("Lagg" = expression(italic("I. aggregata")),   # Italicize names
               "Ihyb" ="Hybrids"), 
   values = c("Lagg" = 16, "Ihyb" = 25))+  # Assign shapes explicitly 
  #labs(subtitle="ANOVA; p=0.031",hjust = 1)+ #adds subtitle at top of figure
    theme(legend.position = "none")  # Removes the legend 
  
    
print(Day.ordination.field)
ggsave(Day.ordination.field, file='plots-nitrogen/Day_ordinationplot_field.jpg', width=130, height=105, units= 'mm', dpi=600)
...
##generate a plot in ggplot
Night.ordination.field = ggplot(bind_cols(filter(cap.night.dataframe.field, score=="sites"),averagemetadata.NIGHT.field), aes(x=CAP1, y= CAP2)) + 
  geom_point(size=2.6, aes(color = treatment, shape = species)) +
  #Compound names on CAP:
  geom_text_repel(data=cap.day.dataframe %>% 
                    left_join(ipochems, by=c("label"="name")) %>% #adds compound name 
 ...
 # ggtitle("") +
  theme(plot.title = element_text(hjust = 0.5))+
  ylab("CAP2 (0.9% explained)") + 
  xlab("CAP1 (14.6% explained)") + 
  #labs(shape = "Species") +
  scale_color_manual(name="Nitrogen Level", 
          labels= c(control="No water added", nitrogen= "High nitrogen", watercontrol= "Low nitrogen"), 
         values=c(control= "#457A26",nitrogen="#E69F00", watercontrol= "#0072B2"),
...
ggsave(Night.ordination.field, file='plots-nitrogen/Night_ordinationplot_field.jpg', width=130, height=105, units= 'mm', dpi=600)

```

#Ordination plot time*species only
```{r}
#GREENHOUSE 

#CAP - species*time
SxTime.cap <- capscale(sqrt(averagetable) ~ time*species, data = averagemetadata, distance = "bray")

#anova of day cap
anova(SxTime.cap, by="term")

#To get % explained (= eigenvalues) for each cap axis
eig.SxTime.cap <-eigenvals(SxTime.cap)
axis.eig.SxTime.cap <- eig.SxTime.cap/ sum(eig.SxTime.cap)
print(axis.eig.SxTime.cap)

cap.SxTime.dataframe <- fortify(SxTime.cap) #turns cap into data frame so you can run it in ggplot


##generate a plot in ggplot
SxTime.ordination = ggplot(bind_cols(filter(cap.SxTime.dataframe, score=="sites"),averagemetadata), aes(x=CAP1, y= CAP2)) + 
  geom_point(size=2.6, aes(color = time, shape = species)) +
  #Compound names on CAP:
  geom_text_repel(data=cap.SxTime.dataframe %>% 
                    left_join(ipochems, by=c("label"="name")) %>% #adds compound name 
              filter(score=="species", CAP1^2 +CAP2^2>0.08^2) %>%  #gets rid of compounds clusterd in middle of cap; EDIT THIS ON FINAL FIGURE**
              mutate(label=greekify(label)),
              aes(x=CAP1*6, y=CAP2*6, label=label), size=3) +
  theme_classic() + 
  #coord_fixed(xlim=c(-1.5, 3), ylim=c(-1.5, 3)) +
  geom_hline(yintercept = 0, linetype="dotted") + 
  geom_vline(xintercept = 0, linetype="dotted") +
 # labs(color = "Time", fill = "Time") +
 # ggtitle("") +
  theme(plot.title = element_text(hjust = 0.5))+
  ylab("CAP2 (2.0% explained)") + 
  xlab("CAP1 (24.7% explained)") + 
  #labs(shape = "Species") +
  scale_color_manual(name="Time of Day", 
          labels= c(D="Day", N= "Night"), 
         values=c(N= "navy", D= "darkorange"),
          guide = guide_legend(order = 1))+  # Treatment legend appears first
  scale_shape_manual(
    name = "Species",  # Capitalize "Species"
    labels = c("agg" = expression(italic("I. aggregata")),   # Italicize names
               "ten" =expression(italic("I. aggregata"))), 
    values = c("agg" = 16, "ten" = 17))  # Assign shapes explicitly 
  #labs(subtitle="ANOVA; p=0.031",hjust = 1)+ #adds subtitle at top of figure
    #theme(legend.position = "none")  # Removes the legend
  
    
print(SxTime.ordination)
ggsave(SxTime.ordination, file='plots-nitrogen/timexspecies_ordinationplot.jpg', width=180, height=150, units= 'mm', dpi=600)

#--------------------------------- field -------------------------------------------------------

#FIELD
#CAP - species*time
SxTime.cap.field <- capscale(sqrt(averagetable.field) ~ time*species, data = averagemetadata.field, distance = "bray")

#anova of day cap
anova(SxTime.cap.field, by="term")

#To get % explained (= eigenvalues) for each cap axis
eig.SxTime.cap.field <-eigenvals(SxTime.cap.field)
axis.eig.SxTime.cap.field <- eig.SxTime.cap.field/ sum(eig.SxTime.cap.field)
print(axis.eig.SxTime.cap.field)

cap.SxTime.dataframe.field <- fortify(SxTime.cap.field) #turns cap into data frame so you can run it in ggplot


##generate a plot in ggplot
SxTime.ordination.field = ggplot(bind_cols(filter(cap.SxTime.dataframe.field, score=="sites"),averagemetadata.field), aes(x=CAP1, y= CAP2)) + 
  geom_point(size=2.6, aes(color = time, shape = species)) +
  #Compound names on CAP:
  geom_text_repel(cap.SxTime.dataframe.field %>% 
                    left_join(ipochems, by=c("label"="name")) %>% #adds compound name 
              filter(score=="species", CAP1^2 +CAP2^2>0.08^2) %>%  #gets rid of compounds clusterd in middle of cap; EDIT THIS ON FINAL FIGURE**
              mutate(label=greekify(label)),
              aes(x=CAP1*6, y=CAP2*6, label=label), size=3) +
 ...
 # ggtitle("") +
  theme(plot.title = element_text(hjust = 0.5))+
  ylab("CAP2 (1.5% explained)") + 
  xlab("CAP1 (27.1% explained)") + 
  #labs(shape = "Species") +
  scale_color_manual(name="Time of Day", 
          labels= c(D="Day", N= "Night"), 
         values=c(N= "navy", D= "darkorange"),
...
print(SxTime.ordination.field)
ggsave(SxTime.ordination.field, file='plots-nitrogen/timexspecies_ordinationplot_field.jpg', width=180, height=150, units= 'mm', dpi=600)
```


#Extract legend from ordination plots
```{r}

megameta.legend <- bind_rows(greenhouse=meta, .id = "experiment", field=meta.field)

#Make ggplot ordination of megameta
(cap_treat <- vegan::capscale(sqrt(mega.vol) ~ time +experiment +species+ treatment+species*treatment+ time*species+time*treatment+ species*treatment*time, data=megameta))
anova.cca(cap_treat, by="term")
# Extract the site scores from the cap_treat object
site_scores <- scores(cap_treat, display = "sites")

# Combine site scores with the original data
megameta_ordination_df <- bind_cols(as.data.frame(site_scores), megameta)



megameta.plot= ggplot(megameta.legend %>% mutate(species=fct_recode(species, Lagg="agg")), aes( x=date, y = time, color = treatment, shape = species)) + 
  theme_classic() + 
  guides(color= guide_legend(order=1 ,override.aes = list(size=5)), shape= guide_legend(order = 2, override.aes = list(size=5)))+
  geom_point()+
  geom_hline(yintercept = 0, linetype="dotted") + 
  geom_vline(xintercept = 0, linetype="dotted") +
  labs(color = "Treatment", fill = "Treatment") +
  ...
  scale_shape_manual(
    name = "Species",  
    labels = c("Lagg" = expression(italic("I. aggregata")),  
               #"agg" = expression(italic("I. aggregata")),  # Same label for both
               "Ihyb" = "Hybrids", 
               "ten" = expression(italic("I. tenuituba"))), 
    values = c("Lagg" = 16,  # Same shape for both Lagg and agg
               "Ihyb" = 25, 
               "ten" = 17))+
   theme(legend.title = element_text(size=18, color = "black"), legend.text=element_text(size=13),
           legend.justification=c(0,1), 
           legend.position = "right",
           #legend.position=c(0.03, 0.95),
           legend.background = element_blank(),
...
print(megameta.plot)
# Extract legends
combined_legend <- cowplot::get_legend(megameta.plot)

#to edit final figures:
#adobe illustrator or inkscape; export plots as .svg (instead of png or jpeg)


# Combine plots and legends
combined_plot <- cowplot::plot_grid(
  combined_legend,
  ncol = 1, nrow = 2, rel_widths = c(1, 0.2), rel_heights = c(1, 0.2)
)

print(combined_plot)
ggsave(combined_plot, file='plots-nitrogen/Legend_plot.jpg', width=130, height=105, units= 'mm', dpi=600)
```
...
plot.total.emissions.pretty=
  ggplot(averagePlant.total, aes( y= total, x=str_to_title(time), color=treatment)) +
  geom_boxplot(position=position_dodge(width=0.9))+
  geom_point (position=position_dodge(width=0.9))+ #makes it so point don't overlap
  scale_y_sqrt()+
  ylab("Total Emissions")+
...
ggsave(plot.total.emissions.pretty, file='plots-nitrogen/greenhouse_totalemissions.jpg', width=150, height=140, units= 'mm', dpi=600)
```
...
plot.total.emissions.field=
  ggplot(averagePlant.total.field, aes( y= total, x=time, color=treatment))+ #could do with average metadata instead of all data points in meta
  geom_boxplot(position=position_dodge(width=0.9))+
  geom_point (position=position_dodge(width=0.9))+ #makes it so point don't overlap
  scale_y_sqrt()+
  ylab("Total Emissions")+
...
print(agg.Day.ordination)
ggsave(agg.Day.ordination, file='plots-nitrogen/agg_Day_ordinationplot.jpg', width=130, height=105, units= 'mm', dpi=600)
...
ggplot(dominantvocs.field, aes(y=fct_reorder(name, value), x=value, color=treatment, shape=time))+
    ...
    scale_color_manual(name="Nitrogen Level", 
          labels= c(control="No water added", nitrogen= "High nitrogen", watercontrol= "Low nitrogen"), 
          values=c(control= "#457A26",nitrogen="#E69F00", watercontrol= "#0072B2")) #manually change colros
    theme_minimal() #removed grey from background
