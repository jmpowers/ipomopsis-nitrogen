...
ipochemsf <-  ipochems %>% mutate(standard = standard_shortnames[standard]) %>% 
  left_join(
...
megameta.ordination= ggplot(megameta_ordination_df, aes(x=CAP1, y= CAP2, color = treatment, shape = species)) + 
  geom_point(size=2.6) +
  theme_classic() + 
  geom_hline(yintercept = 0, linetype="dotted") + 
  geom_vline(xintercept = 0, linetype="dotted") +
  ...
  scale_shape_manual(
    ...
    labels = c("Lagg" = expression(italic("I. aggregata")),  
               ...
               "Ihyb" = "Hybrids", 
               "ten" = expression(italic("I. tenutiba"))), 
    values = c("Lagg" = 16, "agg" = 16,  # Same shape for both Lagg and agg
               "Ihyb" = 25, 
               "ten" = 17))+
   theme(legend.title = element_text(size=12, color = "black"), legend.text=element_text(size=10),
           legend.justification=c(0,1), 
...
print(megameta.ordination)
# Extract legends
greenhouse_legend <- cowplot::get_legend(Day.ordination)
field_legend <- cowplot::get_legend(Day.ordination.field)

# Remove legends from plots
greenhouse_ordination_no_legend <- Day.ordination + theme(legend.position = "none")
field_ordination_no_legend <- Day.ordination.field + theme(legend.position = "none")


# Combine plots and legends
combined_plot <- cowplot::plot_grid(
  greenhouse_legend, field_legend,
  ncol = 2, nrow = 2, rel_widths = c(1, 0.2), rel_heights = c(1, 0.2)
)
...
```{r}
# Create fake data with consistent number of rows (20 rows)
set.seed(123)  # For reproducibility
fake_data <- data.frame(
  CAP1 = rnorm(20),  # Random data for x-axis
  CAP2 = rnorm(20),  # Random data for y-axis
  species = rep(c("Lagg", "Ihyb", "agg", "ten"), length.out = 20),  # Repeat species so there are exactly 20 rows
  treatment = rep(c("control", "nitrogen", "watercontrol"), length.out = 20)  # Repeat treatment so there are exactly 20 rows
)

# Plot using ggplot
ggplot(fake_data, aes(x = CAP1, y = CAP2, color = treatment, shape = species)) + 
  ...
  scale_color_manual(
    name = "Treatment", 
    values = c("control" = "#8b9b31", "nitrogen" = "#b35f98", "watercontrol" = "#3a8db3"), 
    labels = c("No water added", "High nitrogen", "Low nitrogen")
  ) +  # Custom color scale for treatment
  scale_shape_manual(
    name = "Species", 
    values = c("Lagg" = 16, "Ihyb" = 25, "ten" = 17),  # Custom shapes for species
    labels = c("Lagg" = "I. aggregata", "Ihyb" = "Hybrids", "agg" = "Agg", "ten" = "Ten")  # Labels for species
  ) + 
  labs(color = "Treatment", shape = "Species") +  # Customize legend labels
  theme(
    legend.title = element_text(size = 12, color = "black"), 
    legend.text = element_text(size = 10),
    legend.position = "right",  # Place legend on the right side
    plot.title = element_text(hjust = 0.5),  # Center the plot title
    plot.subtitle = element_text(hjust = 1)  # Align the subtitle
  )
```
...
ggsave(plot.total.emissions.field, file='total_emissions_field_boxplot.jpg', width=150, height=140, units= 'mm', dpi=600)
