...
exp1_field_nofilename <- read.csv("data/2022 Poverty Gulch Volatiles.csv") %>%
  mutate(plant = as.character(plant),
         ...
         treatment=fct_recode(replace_na(treatment, "air"), air="", control="ambient")) 
...
metadata.field <- exp1_field_meta %>% #Filename already accounted for skips
  ...
  mutate(plantid = paste(species, plant), speciestime = paste(species, time, sep=".")) %>% 
   mutate(plant = as.character(plant)) %>% 
  rename(sample2=Filename) %>% #to do the join
...
chemtable.field$filter_final <- with(chemtable.field, filter_RT == "OK" & filter_match =="OK" & 
                                 #(filter_freq.agg == "OK" | filter_freq.ten == "OK" | filter_freq.D == "OK" | filter_freq.N == "OK") &
                                 (filter_freq.Lagg.N == "OK" | filter_freq.Ihyb.N == "OK" | filter_freq.Lagg.D == "OK" | filter_freq.Ihyb.D == "OK") &
                                 filter_area == "OK" & filter_ambient_ratio == "OK" & filter_ambient_ttest == "OK" & filter_contaminant == "OK")
...
meta <- metadata.field[metadata.field$type == "floral" & !(metadata.field$sample %in% files_exclude),] %>% droplevels()
vol.qual <- vol.qual / (as.numeric(meta$equil + meta$pumping)/3600) / meta$flrs #0.5 hr of equilibration plus 0.25 hr pumping, one flower
```
...
#vol.field <- vol.exp1.exp1field[rownames(vol.field.qual),]

vol <- vol.exp1.exp1field %>% 
...
betadisper(vegdist(vol), plantid)
