...
morphdata <- read_sheet("1nyUqE-4dVr-81-MFMuAi5-dU4BIYZfuzF8xDEoFyJIk", sheet="Flower Morphology", na=c("","NA")) %>% 
  mutate(corolla_length=as.numeric(corolla_length))%>% left_join(exp1.treatments)
...
#TODO ask arturo for plant treatment list and inflo height?
...
#Model for All the traits - greenhouse
```{r}
library(broom)
...
morph.models <- morphdata %>% pivot_longer(all_of(all.morph.traits), names_to = "trait") %>% group_by(trait) %>% nest() %>% 
      mutate(morph.trait.lmer=map(data, ~lmer(value~treatment+ (1|plantid), data=.x))) %>% 
...
allmorphtraits <- morphdata1 %>% pivot_longer(all_of(all.morph.traits), names_to = "trait") %>% 
    ...
    ggplot(aes(color= str_to_title(treatment), x=str_to_title(treatment), y=value))+geom_boxplot()+
...
#Model for All the traits - field
```{r}
library(broom)
...
morph.models.field <- morphdata.field %>% pivot_longer(all_of(all.morph.traits.field), names_to = "trait") %>% group_by(trait) %>% nest() %>% 
      mutate(morph.trait.field.lmer=map(data, ~lmer(value~treatment+ (1|plantid), data=.x))) %>% 
      mutate(morph.field.Anova= map(morph.trait.field.lmer, ~tidy(Anova(.x, type=3)))) %>%  unnest(morph.field.Anova) %>%
      dplyr::select(-data,-morph.trait.field.lmer) %>% 
      mutate(significant = ifelse(p.value<0.05, "*","") )
...
# read in nectar data and find averages - greenhouse
```{r}
