...
NEXP1.theme <- theme(
      ...
      #facets
                  ...
                  strip.text = element_text(size = 13)        #This changes the size of the facet labels
...
allmorphtraits <- morphdata1 %>% pivot_longer(all_of(all.morph.traits), names_to = "trait") %>% 
    ...
    ggplot(aes(color= str_to_title(treatment), x=str_to_title(species), y=value))+
        ...
        labs(y="Length (mm)", x="Species", color="Nitrogen") +
    NEXP1.theme+
   #scale_y_sqrt()+
...
ggsave(allmorphtraits, file="plots-nitrogen/allmorphtraits.png", width= 4.5, height=5.5)
...
ggsave(allmorphtraits.field, file="plots-nitrogen/NEXP1.field.allmorphtraits.png", width= 4.5, height=5.5, dpi=600)
...
nectar.plot <- nectardata1 %>% pivot_longer(all_of(nectar.traits), names_to = "trait") %>% #need to pivot longer for facets
    ...
    ylab("") +
   scale_color_manual(values = c("skyblue", "orange"))+theme_minimal()+
  theme(panel.grid.major.x = element_blank(), legend.position = "bottom")+ 
   labs(y="", x="Species", color="Nitrogen")
print(nectar.plot)
ggsave(nectar.plot, file='greenhouse_nectar.jpg', width=150, height=140, units= 'mm', dpi=300)
...
nectar.field.plot <- nectardata1.field %>% pivot_longer(all_of(nectar.traits.field), names_to = "trait") %>% #need to pivot longer for facets
    ...
    facet_wrap(vars(trait),scales = "free_y")+
    ylab("") +
    scale_color_manual(values = c("forestgreen","orange","skyblue"))+theme_minimal()+
  theme(panel.grid.major.x = element_blank(), legend.position = "bottom")+ 
   labs(y="", x="Species", color="Nitrogen")
print(nectar.field.plot)
ggsave(nectar.field.plot, file='field_nectar.jpg', width=150, height=140, units= 'mm', dpi=300)
...
flower.number.plot<- ggplot(total.flower, aes(y=flower_num, x=species, color=treatment))+geom_boxplot()+
  scale_color_manual(values = c("skyblue", "orange"))+theme_minimal()+
  theme(panel.grid.major.x = element_blank(), legend.position = "bottom")+ 
   labs(y="Flower Number", x="Species", color="Nitrogen")
print(flower.number.plot)
ggsave(flower.number.plot, file="plots-nitrogen/NEXP1.greenhouse.flowernumber.png", width= 4.5, height=5.5)
